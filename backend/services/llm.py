...
async def stream_text(
    ...
    site_url: Optional[str] = os.getenv(
        "OPENROUTER_SITE_URL", None
    ),  # Optional: For leaderboard ranking
    site_title: Optional[str] = os.getenv(
        "OPENROUTER_SITE_TITLE", None
    ),  # Optional: For leaderboard ranking
    response_schema: Optional[
        Dict[str, Any]
    ] = None,  # New: JSON schema for structured output
    schema_name: Optional[
        str
    ] = None,  # New: Optional name for the schema (used in OpenRouter's format)
    ...
    if should_use_ollama:
        ...
        ollama_model = (
            model if model != default_model else os.getenv("OLLAMA_MODEL", "llama3")
        )
    ...
    if should_use_anakin:
        ...
        async for chunk in stream_text_anakin(
            prompt, model, max_tokens, system_prompt, messages, callback
        ):
            yield chunk
        return
    ...
    try:
        ...
        if system_prompt and not any(
            msg["role"] == "system" for msg in messages_config
        ):
            logger.debug("Prepending system prompt")
            messages_config.insert(0, {"role": "system", "content": system_prompt})
        elif system_prompt:
            logger.warning(
                "System prompt provided but messages_config already contains a system message. Ignoring provided system_prompt argument."
            )
        ...
        if response_schema:
            ...
            if not schema_name:
                logger.warning(
                    "No schema_name provided for structured output, using default: 'custom_schema'"
                )
            ...
            logger.info(
                f"Using structured output with schema name: {schema_name_to_use}, strict: {schema_strict}"
            )
        ...
        if "response_format" in stream_params:
            loggable_params["response_format_type"] = stream_params[
                "response_format"
            ].get("type")
            loggable_params["schema_name"] = (
                stream_params["response_format"].get("json_schema", {}).get("name")
            )
        if extra_body:
            loggable_params["extra_body"] = extra_body  # Log extra_body content
        logger.debug(
            f"Stream parameters (excluding messages/schema): {loggable_params}"
        )

        try:
            # Pass extra_body to the create call
            stream = await client.chat.completions.create(
                **stream_params, extra_body=extra_body if extra_body else None
            )
            ...
            async for chunk in stream:
                ...
                if chunk.choices and hasattr(chunk.choices[0].delta, "reasoning"):
                    ...
                    if reasoning_content:
                        logger.debug(
                            f"Chunk contains reasoning delta: {reasoning_content}"
                        )
                # --- END ADDED ---
            ...
            logger.error(
                f"Error during async streaming with OpenRouter: {str(stream_error)}",
                exc_info=True,
            )
            # Check if the error message indicates lack of support for reasoning *parameter* specifically
            if extra_body.get("reasoning") is not None and (
                "reasoning" in str(stream_error).lower()
                or "support" in str(stream_error).lower()
            ):
                logger.warning(
                    f"Model '{model}' might not support the 'reasoning' parameter via extra_body, or the parameter structure is incorrect."
                )
            # Check for structured output errors
            if (
                "response_format" in stream_params
                and "support" in str(stream_error).lower()
            ):
                logger.warning(
                    f"Model '{model}' might not support structured outputs ('response_format'), or the schema might be invalid."
                )
            raise

    except Exception as e:
        logger.error(
            f"Error in async stream_text (OpenRouter): {str(e)}", exc_info=True
        )
...
async def stream_sambanova(
    ...
    ),  # Default from quickstart if not in settings
    ...
    try:
        ...
        sambanova_base_url = (
            "https://api.sambanova.ai/v1"  # As per SambaNova documentation
        )

        logger.debug(
            f"Initializing AsyncOpenAI client for SambaNova: {sambanova_base_url}"
        )
        client = AsyncOpenAI(
        ...
        if messages is None:
            ...
            logger.debug(
                f"Using provided messages array with {len(messages)} messages for SambaNova"
            )
            messages_config = messages
        ...
        if system_prompt and not any(
            ...
            logger.warning(
                "System prompt provided for SambaNova but messages_config already contains a system message. Ignoring provided system_prompt argument."
            )
        ...
        logger.debug(
            f"SambaNova Stream parameters (excluding messages): {loggable_params}"
        )
        ...
        try:
            ...
            logger.error(
                f"Error during async streaming with SambaNova: {str(stream_error)}",
                exc_info=True,
            )
            raise
...
async def stream_ollama(
    ...
    try:
        ...
        if messages is None:
            ...
            logger.debug(
                f"Using provided messages array with {len(messages)} messages for Ollama"
            )
            messages_config = messages
        ...
        loggable_params = {k: v for k, v in stream_params.items() if k != "messages"}
        logger.debug(
            f"Ollama Stream parameters (excluding messages): {loggable_params}"
        )
        ...
        try:
            ...
            logger.error(
                f"Could not connect to Ollama at {ollama_base_url}. Is it running? Error: {e}",
                exc_info=True,
            )
            raise
        except Exception as stream_error:
            logger.error(
                f"Error during async streaming with Ollama: {str(stream_error)}",
                exc_info=True,
            )
            raise
...
async def stream_text_anakin(
    ...
    try:
        ...
        if not app_id:
            ...
            if not app_id:
                logger.error(
                    "ANAKIN_APP_ID is not configured in settings and not provided"
                )
        ...
        if messages:
            ...
            for msg in messages:
                ...
                if (
                    role == "system" and not system_prompt
                ):  # Only add if no system_prompt was already added
        ...
        # Prepare payload
        payload = {"content": content.strip(), "stream": True}
        if thread_id:
        ...
        logger.debug(
            f"Anakin request payload (content length: {len(payload['content'])})"
        )
...
async def get_json(
    ...
    if should_use_ollama:
        ...
        try:
            ...
            logger.error(
                f"Failed to parse JSON from Ollama response: {e}", exc_info=True
            )
            logger.debug(
                f"Ollama response content: {response.choices[0].message.content}"
            )
        ...
        try:
            ...
            logger.error(
                f"Failed to parse tool call from LLM response: {e}", exc_info=True
            )
            logger.debug(f"LLM response object: {response}")
            raise ValueError(
                "Could not extract a valid JSON tool call from the LLM response."
            )
