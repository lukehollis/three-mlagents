...
class FishEnv:
    ...
    def reset(self):
        ...
        self.shark_pos = np.random.uniform(
            SHARK_RADIUS + 5, GRID_SIZE - SHARK_RADIUS - 5, size=3
        )
        ...
        for i in range(NUM_FISH):
            ...
            while attempts < 100:  # Max attempts to find valid position
                candidate_pos = np.random.uniform(
                    FISH_RADIUS + 2, GRID_SIZE - FISH_RADIUS - 2, size=3
                )
            ...
            if attempts >= 100:
                self.fish_pos[i] = np.random.uniform(
                    FISH_RADIUS + 2, GRID_SIZE - FISH_RADIUS - 2, size=3
                )
                ...
                for j in range(i + 1, NUM_FISH):
                    ...
                    if distance < FISH_SPACING:
                        ...
                        if distance > 1e-6:  # Avoid division by zero
                            ...
                            separation_vector = separation_vector / np.linalg.norm(
                                separation_vector
                            )
                        ...
                        adjusted_positions[i] = np.clip(
                            adjusted_positions[i], FISH_RADIUS, GRID_SIZE - FISH_RADIUS
                        )
                        adjusted_positions[j] = np.clip(
                            adjusted_positions[j], FISH_RADIUS, GRID_SIZE - FISH_RADIUS
                        )
    ...
    def _get_state(self):
        ...
        for i in range(NUM_FISH):
            ...
            distances_to_all_food = [
                np.linalg.norm(self.fish_pos[i] - food_pos)
                for food_pos in self.food_pos
            ]
            min_food_distance = min(distances_to_all_food)
            normalized_food_distance = min_food_distance / (
                GRID_SIZE * np.sqrt(3)
            )  # Normalize to 0-1

            # Distance to shark
            shark_distance = np.linalg.norm(self.fish_pos[i] - self.shark_pos)
            normalized_shark_distance = shark_distance / (
                GRID_SIZE * np.sqrt(3)
            )  # Normalize to 0-1

            states.append(
                np.array(
                    [normalized_food_distance, normalized_shark_distance],
                    dtype=np.float32,
                )
            )
        return states
    ...
    def step(self, actions):
        ...
        for i in range(NUM_FISH):
            ...
            food_direction = food_direction / (
                np.linalg.norm(food_direction) + 1e-8
            )  # Normalize
            ...
            intended_positions[i] = np.clip(
                intended_positions[i], FISH_RADIUS, GRID_SIZE - FISH_RADIUS
            )
        ...
        if NUM_FISH > 0:
            ...
            distances_to_fish = [
                np.linalg.norm(self.shark_pos - fish_pos) for fish_pos in self.fish_pos
            ]
            ...
            shark_direction = shark_direction / (
                np.linalg.norm(shark_direction) + 1e-8
            )  # Normalize
            shark_movement = shark_direction * SHARK_SPEED

            self.shark_pos += shark_movement
            self.shark_pos = np.clip(
                self.shark_pos, SHARK_RADIUS, GRID_SIZE - SHARK_RADIUS
            )
        ...
        for i in range(NUM_FISH):
            ...
            # Check if food is eaten (only if not caught by shark)
                ...
                reward = REWARD_FOOD
                self.fish_energy[i] = min(
                    MAX_ENERGY, self.fish_energy[i] + ENERGY_FROM_FOOD
                )
                ...
                actual_movement_distance = np.linalg.norm(
                    self.fish_pos[i] - old_positions[i]
                )
                food_distance_improvement = old_food_distances[i] - new_food_distance
                shark_distance_improvement = (
                    new_shark_distance - old_shark_distances[i]
                )  # Positive when moving away from shark
                ...
                reward += (
                    shark_distance_improvement * 15.0
                ...
                for j in range(NUM_FISH):
                    if i != j:
                        fish_distance = np.linalg.norm(
                            self.fish_pos[i] - self.fish_pos[j]
                        )
            ...
            if fish_caught_by_shark or self.fish_energy[i] <= 0:
                ...
                for attempt in range(100):
                    ...
                    for j in range(NUM_FISH):
                        if (
                            i != j
                            and np.linalg.norm(candidate_pos - self.fish_pos[j])
                            < FISH_SPACING
                        ):
                            valid_pos = False
                            break
            ...
            agents.append(
                {
                    "id": i,
                    "pos": [
                        float(self.fish_pos[i, 0]),
                        float(self.fish_pos[i, 1]),
                        float(self.fish_pos[i, 2]),
                    ],
