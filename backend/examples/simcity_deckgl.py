...
from services.llm import get_json, get_embedding  # noqa: E402
...
class Building:
    def __init__(
        self, building_id: int, pos: np.ndarray, building_type: str, initiator_id: int
    ):
        self.id = building_id
        self.pos = pos
    ...
    def contribute_resource(
        self, resource: str, amount: int, contributor_id: int
    ) -> int:
...
class Business:
    def __init__(self, business_id: int, pos: np.ndarray, business_type: str):
        ...
        self.inventory = {
            resource: random.randint(10, 50) for resource in RESOURCES.keys()
        }
        self.prices = {
            resource: RESOURCES[resource]["value"] * random.uniform(0.8, 1.2)
            for resource in RESOURCES.keys()
        }
        self.customers_served = 0
        self.revenue = 0.0
        self.resource_generation_rate = {
            resource: random.uniform(0.1, 0.5) for resource in RESOURCES.keys()
        }
    ...
    def serve_customer(
        self, customer_needs: Dict[str, int]
...
class Pedestrian:
    def __init__(
        self, ped_id: int, start_pos: np.ndarray, graph: "networkx.MultiDiGraph"
    ):
        self.id = ped_id
        self.graph = graph
        ...
        self.resources = {
            resource: random.randint(5, 15) for resource in RESOURCES.keys()
        }
        ...
        self.state = random.choice(
            ["wandering", "gathering", "working", "building", "communicating"]
        )
        self.state_timer = 0
        ...
        if target_node and self.current_node:
            try:
                path = ox.shortest_path(
                    self.graph, self.current_node, target_node, weight="length"
                )
    ...
    async def decide_action_llm(
        ...
        current_projects = [
            b for b in buildings if b.status in ["planning", "under_construction"]
        ]
        ...
        prompt = f"""You are Agent {
            self.id
        } in a collaborative city building simulation.
...
- Current project: {
            self.current_building_project
            if self.current_building_project is not None
            else "None"
        }
- State: {self.state}
...
- Nearby businesses: {
            len([b for b in businesses if np.linalg.norm(self.pos - b.pos) < 0.001])
        }
                ...
                [
                    {
                        ...
                        "progress": b.progress,
                        "resources_needed": {
                            k: v - b.resources_contributed[k]
                            for k, v in b.resources_needed.items()
                            if v > b.resources_contributed[k]
                        },
...
Respond with valid JSON only, no explanations."""
        ...
        log_to_frontend(
            f"🧠 Agent {self.id}: LLM response - action: {response.get('action')}, data: {response.get('data')}"
        )
        ...
        conversation_text = conversation_response.get(
            "message", f"Agent {self.id}: Planning my next move..."
        )
        ...
        if _current_websocket:
            try:
                ...
                logger.debug(f"Failed to send conversation message: {e}")
    ...
    def get_fast_action(
        self,
        trained_policy: object | None,
        businesses: List[Business],
        buildings: List[Building],
    ) -> Tuple[str, Any]:
        """Get action using trained RL policy or LLM intent"""
        log_to_frontend(
            f"🎯 Agent {self.id}: get_fast_action called, llm_intent = {self.llm_intent}"
        )
        ...
        if trained_policy:
            ...
            if action_name in ACTION_MAP_MOVE:
                ...
                nearest_business = min(
                    businesses, key=lambda b: np.linalg.norm(self.pos - b.pos)
                )
                ...
                return (
                    "start_building",
                    {
                        "building_type": self.building_goal,
                        "reason": "AI policy decision",
                    },
                )
                ...
                active_projects = [
                    b
                    for b in buildings
                    if b.status in ["planning", "under_construction"]
                ]
                if active_projects:
                    target_building = min(
                        active_projects, key=lambda b: np.linalg.norm(self.pos - b.pos)
                    )
                    contribution = {}
                    for resource, needed in target_building.resources_needed.items():
                        ...
                        if (
                            available > 0
                            and target_building.resources_contributed[resource] < needed
                        ):
                            contribution[resource] = min(
                                available,
                                needed
                                - target_building.resources_contributed[resource],
                            )
                    return (
                        "contribute_building",
                        {"building_id": target_building.id, "resources": contribution},
                    )
            elif action_name == "communicate":
                messages = [
                    f"I'm looking to build a {self.building_goal}",
                    "Anyone need help with construction?",
                    f"I have {sum(self.resources.values())} resources to share",
                    "Let's work together on a big project!",
                ]
                return (
                    "communicate",
                    {"message": random.choice(messages), "recipient_id": None},
                )
    ...
    def _get_state_vector(
        self, businesses: List[Business], buildings: List[Building]
    ) -> np.ndarray:
        ...
        resource_vector = np.array(
            [self.resources.get(r, 0) / 100.0 for r in RESOURCES.keys()]
        )
        ...
        if businesses:
            nearest_business_dist = min(
                [np.linalg.norm(self.pos - b.pos) for b in businesses]
            )
    ...
    def _update_satisfaction(self, buildings: List[Building]):
        ...
        self.satisfaction = max(
            0, min(100, resource_satisfaction + building_bonus + community_bonus)
        )
    ...
    def update_memory(self, text: str):
        ...
        self.memory_vector = (self.memory_vector * 0.9) + (
            new_embedding.astype(np.float32) * 0.1
        )
...
class SimCityEnv:
    def __init__(
        self, training_mode: bool = True, trained_policy: object | None = None
    ):
        ...
        self.running = False
        self.trained_policy: object | None = trained_policy
        ...
        dist = 4500
        bounds = {
            "north": self.location_point[0] + 0.01,
            "south": self.location_point[0] - 0.01,
            "east": self.location_point[1] + 0.01,
            "west": self.location_point[1] - 0.01,
            "dist": dist,
        }
        ...
        if cached_graph:
            ...
            self.graph = ox.graph_from_point(
                self.location_point, dist=dist, network_type="drive"
            )
            if redis_client:
                try:
                    redis_client.set(
                        cache_key, pickle.dumps(self.graph), ex=CACHE_EXPIRY_SECONDS
                    )
    ...
    def set_trained_policy(self, policy: object | None):
    ...
    def _get_dummy_obs(self):
        ...
        return {
            ...
            "agent_resources": np.array(
                [
                    [p.resources.get(r, 0) for r in RESOURCES.keys()]
    ...
    def _get_obs_for_agent(self, agent: Pedestrian) -> Dict[str, np.ndarray]:
        ...
        # Resources
        resource_vector = np.array(
            [agent.resources.get(r, 0) for r in RESOURCES.keys()], dtype=np.float32
        )
        ...
        if self.businesses:
            nearest_business_dist = min(
                [np.linalg.norm(agent.pos - b.pos) for b in self.businesses]
            )
        else:
        ...
        env_vector = np.array(
            [nearest_business_dist, active_projects], dtype=np.float32
        )
    ...
    def step(self, action: int):
        ...
        for i, agent in enumerate(self.pedestrians):
            if i == 0:  # Only use action for first agent
                ...
                if self.trained_policy:
                    state_vector = agent._get_state_vector(
                        self.businesses, self.buildings
                    )
        ...
        info = {
            "buildings": len(self.buildings),
            "completed_buildings": len(
                [b for b in self.buildings if b.status == "completed"]
            ),
    ...
    def _create_action_data(self, agent: Pedestrian, action_name: str) -> Dict:
        ...
        if action_name in ACTION_MAP_MOVE:
            ...
            if self.businesses:
                nearest_business = min(
                    self.businesses, key=lambda b: np.linalg.norm(agent.pos - b.pos)
                )
            ...
            if active_projects:
                target_building = min(
                    active_projects, key=lambda b: np.linalg.norm(agent.pos - b.pos)
                )
                contribution = {}
                for resource, needed in target_building.resources_needed.items():
                    ...
                    if (
                        ...
                        contribution[resource] = min(
                            available,
                            needed - target_building.resources_contributed[resource],
                        )
    ...
    def _create_traffic_lights(self):
        ...
        selected_intersections = random.sample(
            intersections, min(NUM_TRAFFIC_LIGHTS, len(intersections))
        )
    ...
    def _find_valid_building_spot(
        self, agent_pos: np.ndarray, max_tries=50
    ) -> np.ndarray | None:
        ...
        for _ in range(max_tries):
            ...
            try:
                ...
                _, _, _, dist_to_road = ox.distance.nearest_edges(
                    self.graph, X=candidate_pos[1], Y=candidate_pos[0], return_dist=True
                )
                ...
                for u, v, data in self.graph.edges(data=True):
                    if (
                        "geometry" in data
                        and candidate_point.distance(data["geometry"]) < min_road_dist
                    ):
                        is_on_road = True
                        break
    ...
    def _calculate_reward(self) -> float:
        ...
        building_value = sum(
            [b.base_value for b in self.buildings if b.status == "completed"]
        )
        ...
        collaboration_bonus = (
            len([b for b in self.buildings if len(b.contributors) > 1]) * 50
        )
    ...
    def _get_reward(self, agent: Pedestrian, action: str, data: Any) -> float:
        ...
        if action == "move":
            ...
            if (
                unfinished_buildings > len(self.pedestrians) // 3
            ):  # More than 1/3 of agents worth of buildings
                reward -= base_value * 0.5  # Large penalty
                reward -= (
                    unfinished_buildings * 10
                )  # Additional penalty per unfinished building
            else:
                reward += (
                    base_value * 0.1
                )  # Reward for initiating valuable projects only when not too many active
            ...
            if building:
                ...
                if building.status == "planning":
                    ...
                    total_remaining_value = sum(
                        [
                            RESOURCES[r]["value"] * amount
                            for r, amount in remaining_resources.items()
                        ]
                    )
                    covered_value = sum(
                        [
                            RESOURCES[r]["value"]
                            * min(amount, remaining_resources.get(r, 0))
                            for r, amount in resources_contributed.items()
                        ]
                    )
                    ...
                    if total_remaining_value > 0:
                        ...
                        if all(
                            building.resources_contributed[r]
                            + resources_contributed.get(r, 0)
                            >= needed
                    ...
                    reward += base_contribution_reward * (
                        1.0 + progress_ratio
                    )  # More reward for buildings closer to completion
    ...
    def _execute_move_action(self, agent: Pedestrian, data: Dict):
        ...
        if "move_vector" in data:
            ...
            agent.add_to_memory_stream(
                f"Moved by vector {move_vector.tolist()}", self.step_count
            )
            ...
            if business:
                ...
                agent.add_to_memory_stream(
                    f"Moving towards business {business_id}", self.step_count
                )
            ...
            if building:
                ...
                agent.add_to_memory_stream(
                    f"Moving towards building {building_id}", self.step_count
                )
        else:
            # Random exploration
            all_nodes = list(self.graph.nodes())
    ...
    def _execute_gather_action(self, agent: Pedestrian, data: Dict):
        ...
        if business and np.linalg.norm(agent.pos - business.pos) < 0.0005:
            ...
            resources_needed = {
                ...
                for r in random.sample(list(RESOURCES.keys()), 2)
            }
            ...
            if provided and agent.resources["money"] >= cost:
                ...
                log_to_frontend(
                    f"💰 Agent {agent.id} gathered {provided} for ${cost:.1f}"
                )
                agent.add_to_memory_stream(
                    f"Gathered {provided} from business {business_id} for ${cost:.1f}",
                    self.step_count,
                )
            else:
                agent.add_to_memory_stream(
                    f"Failed to gather resources from business {business_id} - insufficient funds",
                    self.step_count,
                )
        else:
            ...
            if business:
                agent.set_destination(business.pos)
                agent.add_to_memory_stream(
                    f"Moving towards business {business_id} to gather resources",
                    self.step_count,
                )
    ...
    def _execute_work_action(self, agent: Pedestrian, data: Dict):
        ...
        if business and np.linalg.norm(agent.pos - business.pos) < 0.0005:
            ...
            agent.add_to_memory_stream(
                f"Worked at business {business_id} and earned ${wage:.1f}",
                self.step_count,
            )
        else:
            if business:
                agent.set_destination(business.pos)
                agent.add_to_memory_stream(
                    f"Moving towards business {business_id} to work", self.step_count
                )
    ...
    def _execute_start_building_action(self, agent: Pedestrian, data: Dict):
        ...
        if building_type not in BUILDING_RECIPES:
            agent.add_to_memory_stream(
                f"Failed to start building - unknown type: {building_type}",
                self.step_count,
            )
            return
        ...
        if agent.current_building_project is not None:
            ...
            current_building = next(
                (b for b in self.buildings if b.id == agent.current_building_project),
                None,
            )
            if current_building and current_building.status != "completed":
                agent.add_to_memory_stream(
                    f"Cannot start new building - already working on building {agent.current_building_project}",
                    self.step_count,
                )
                log_to_frontend(
                    f"🚫 Agent {agent.id} tried to start {building_type} but already working on building {agent.current_building_project}"
                )
                return
            else:
        ...
        active_buildings = [
            b for b in self.buildings if b.status in ["planning", "under_construction"]
        ]
        max_concurrent_buildings = max(
            3, len(self.pedestrians) // 2
        )  # At least 3, or half the number of agents

        if len(active_buildings) >= max_concurrent_buildings:
            agent.add_to_memory_stream(
                f"Cannot start new building - too many active projects ({len(active_buildings)}/{max_concurrent_buildings})",
                self.step_count,
            )
            log_to_frontend(
                f"🚫 Agent {agent.id} tried to start {building_type} but too many active projects ({len(active_buildings)}/{max_concurrent_buildings})"
            )
            return
        ...
        if building_pos is None:
            agent.add_to_memory_stream(
                f"Failed to find a valid spot to build a {building_type}",
                self.step_count,
            )
            return
        ...
        log_to_frontend(
            f"🏗️ Agent {agent.id} started building project: {building_type} (#{building_id})"
        )
        agent.add_to_memory_stream(
            f"Started {building_type} building project (ID: {building_id}). Reason: {reason}",
            self.step_count,
        )
        ...
        self.add_message(
            agent.id,
            f"I've started a {building_type} project! Who wants to help? Reason: {reason}",
        )
    ...
    def _execute_contribute_building_action(self, agent: Pedestrian, data: Dict):
        ...
        if not building:
            agent.add_to_memory_stream(
                f"Failed to contribute - building {building_id} not found",
                self.step_count,
            )
            return
        ...
        if building.status == "completed":
            agent.add_to_memory_stream(
                f"Building {building_id} already completed", self.step_count
            )
        ...
        for resource, amount in resources_to_contribute.items():
            if agent.resources.get(resource, 0) >= amount:
                actual_contribution = building.contribute_resource(
                    resource, amount, agent.id
                )
                if actual_contribution > 0:
                    agent.resources[resource] -= actual_contribution
                    total_contributed[resource] = actual_contribution
                    contribution_value += (
                        RESOURCES[resource]["value"] * actual_contribution
                    )

        if total_contributed:
            # Update current project if not set or if contributing to a different building
            if (
                agent.current_building_project is None
                or agent.current_building_project != building_id
            ):
                ...
                if agent.current_building_project is not None:
                    old_building = next(
                        (
                            b
                            for b in self.buildings
                            if b.id == agent.current_building_project
                        ),
                        None,
                    )
                    if old_building and old_building.status == "completed":
                        log_to_frontend(
                            f"✅ Agent {agent.id}'s project {agent.current_building_project} completed, switching to {building_id}"
                        )

                agent.current_building_project = building_id
                agent.add_to_memory_stream(
                    f"Switched focus to building project {building_id}", self.step_count
                )
            ...
            log_to_frontend(
                f"🔨 Agent {agent.id} contributed {total_contributed} to building {building_id}"
            )
            agent.add_to_memory_stream(
                f"Contributed {total_contributed} to {building.type} project (value: ${contribution_value})",
                self.step_count,
            )
            ...
            if building.status == "planning" and building.can_start_construction():
                log_to_frontend(
                    f"🚀 Building {building_id} ({building.type}) can now start construction!"
                )
                agent.add_to_memory_stream(
                    f"Building {building_id} ready for construction thanks to my contribution!",
                    self.step_count,
                )
            ...
            self.add_message(
                agent.id,
                f"I contributed {total_contributed} to the {building.type} project! Progress continues.",
            )
        else:
            agent.add_to_memory_stream(
                f"No resources available to contribute to building {building_id}",
                self.step_count,
            )
    ...
    def _execute_communicate_action(self, agent: Pedestrian, data: Dict):
        ...
        if recipient_id is not None:
            recipient = next(
                (a for a in self.pedestrians if a.id == recipient_id), None
            )
            if recipient:
                recipient.update_memory(f"Agent {agent.id} said: {message}")
                recipient.add_to_memory_stream(
                    f"Received message from Agent {agent.id}: '{message}'",
                    self.step_count,
                )
                agent.add_to_memory_stream(
                    f"Sent message to Agent {recipient_id}: '{message}'",
                    self.step_count,
                )
        else:
            # Broadcast
            for other_agent in self.pedestrians:
                if other_agent.id != agent.id:
                    ...
                    other_agent.add_to_memory_stream(
                        f"Heard Agent {agent.id} say: '{message}'", self.step_count
                    )
            agent.add_to_memory_stream(
                f"Broadcast message: '{message}'", self.step_count
            )
    ...
    async def async_step(self):
        ...
        current_thinking_count = sum(
            1 for agent in self.pedestrians if agent.is_thinking
        )
        ...
        agent_actions = []
        for agent in self.pedestrians:
            action = agent.get_fast_action(
                self.trained_policy, self.businesses, self.buildings
            )
        ...
        for building in self.buildings:
            if building.advance_construction():
                log_to_frontend(
                    f"🏗️ Building {building.id} ({building.type}) completed!"
                )
                ...
                for contributor_id in building.contributors:
                    contributor = next(
                        (p for p in self.pedestrians if p.id == contributor_id), None
                    )
                    if contributor:
                        contributor.add_to_memory_stream(
                            f"Building {building.id} ({building.type}) completed! I was a contributor.",
                            self.step_count,
                        )
                        ...
                        if contributor.current_building_project == building.id:
                            ...
                            log_to_frontend(
                                f"✅ Agent {contributor.id} completed their building project {building.id}"
                            )
...
def get_agent_state_vector(
    agent: Pedestrian, businesses: List[Business], buildings: List[Building]
) -> np.ndarray:
    """Create state vector for RL policy"""
...
def get_valid_actions_mask(
    ...
    if (
        sum(agent.resources.values()) < 10
        or agent.current_building_project is not None
        or len(active_buildings) >= max_concurrent_buildings
    ):
