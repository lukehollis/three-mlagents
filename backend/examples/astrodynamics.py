...
class AstrodynamicsEnv(gym.Env):
    ...
    def __init__(self, training_mode: bool = True):
        ...
        self.target_pos = np.zeros(
            3
        )  # target space station position (in relative frame, so always 0)
        self.target_vel = np.zeros(
            3
        )  # target space station velocity (in relative frame, so always 0)
    ...
    def _get_derivatives(
        self, state: np.ndarray, thrust_vector: np.ndarray, current_mass: float
    ) -> np.ndarray:
        """
    ...
    def reset(self, seed=None, options=None):
        ...
        start_scenario = self.np_random.choice(["leo", "outer_orbit"])
        ...
        if start_scenario == "leo":
            ...
            outer_radius = self.np_random.uniform(
                self.orbit_radius * 1.2, self.orbit_radius * 2.5
            )
    ...
    def step(self, action: int):
        ...
        up_dir = self.spacecraft_pos_abs / (
            np.linalg.norm(self.spacecraft_pos_abs) + 1e-8
        )
        ...
        k2_sc = self._get_derivatives(
            state_sc + 0.5 * self.dt * k1_sc, thrust_vector, current_mass
        )
        k3_sc = self._get_derivatives(
            state_sc + 0.5 * self.dt * k2_sc, thrust_vector, current_mass
        )
        k4_sc = self._get_derivatives(
            state_sc + self.dt * k3_sc, thrust_vector, current_mass
        )
        self.spacecraft_pos_abs += (self.dt / 6.0) * (
            k1_sc[0:3] + 2 * k2_sc[0:3] + 2 * k3_sc[0:3] + k4_sc[0:3]
        )
        self.spacecraft_vel_abs += (self.dt / 6.0) * (
            k1_sc[3:6] + 2 * k2_sc[3:6] + 2 * k3_sc[3:6] + k4_sc[3:6]
        )
        ...
        self.target_pos_abs += (self.dt / 6.0) * (
            k1_t[0:3] + 2 * k2_t[0:3] + 2 * k3_t[0:3] + k4_t[0:3]
        )
        self.target_vel_abs += (self.dt / 6.0) * (
            k1_t[3:6] + 2 * k2_t[3:6] + 2 * k3_t[3:6] + k4_t[3:6]
        )
        ...
        if spacecraft_radius < self.earth_radius:
            ...
            distance < self.docking_threshold and velocity_mag > self.velocity_threshold
        ):
        ...
        if distance > self.docking_approach_threshold:
            ...
            radius_reward = (
                np.exp(-((radius_diff / (altitude_scale * 0.1)) ** 2)) * 50.0
            )
            reward += radius_reward

            # 2. Orbital Velocity Reward
            radial_velocity_component = np.dot(self.spacecraft_vel_abs, up_dir) * up_dir
            tangential_velocity_vec = (
                self.spacecraft_vel_abs - radial_velocity_component
            )
            ...
            velocity_match_reward = (
                np.exp(-((velocity_diff / (self.orbital_velocity * 0.15)) ** 2)) * 40.0
            )
            ...
            distance_reward = (
                1.0 - (distance / self.docking_approach_threshold)
            ) * 25.0
            reward += distance_reward

            # 2. Velocity matching reward (critical at close range)
            velocity_reward = (
                np.exp(-((velocity_mag / self.velocity_threshold) ** 2)) * 50.0
            )
    ...
    def _get_obs(self):
        ...
        vel_norm = (
            self.relative_vel / 10000.0
        )  # Normalize by a typical orbital velocity delta
...
async def send_json_safely(websocket: WebSocket, payload: dict) -> bool:
    """Safely send JSON over a websocket, returning True on success."""
...
async def train_astrodynamics(websocket: WebSocket):
    ...
    await train_task_for_websocket(websocket, "astrodynamics")


async def run_simulation(websocket: WebSocket):
    """Run a physics-only no-thrust preview and stream visualization state."""
...
async def run_astrodynamics(websocket: WebSocket, model_filename: str | None = None):
    ...
    await run_policy_for_websocket(
        websocket,
        "astrodynamics",
        lambda: AstrodynamicsEnv(training_mode=False),
