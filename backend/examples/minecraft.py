...
try:
    ...
    async def get_json(*args, **kwargs):
        raise RuntimeError(
            "Minecraft LLM actions require the optional services.llm dependencies."
...
CRAFTING_RECIPES = {
    ...
    "diamond_pickaxe": {
        "craft_time": 10,
        "value": 1000,
        "recipe": {"diamond": 3, "obsidian": 2},
    },
    "crystal_wand": {
        "craft_time": 15,
        "value": 3000,
        "recipe": {"crystal": 5, "gold": 2, "wood": 1},
    },
}
...
class Agent:
    def __init__(self, agent_id: int, pos: np.ndarray):
        ...
        self.inventory = {
            res: 0
            for res in list(RESOURCE_TYPES.keys()) + list(CRAFTING_RECIPES.keys())
        }
        ...
        self.last_llm_step = (
            -10
        )  # Last step when LLM was called (start with -10 for immediate first call)
        self.memory_vector = np.zeros(
    ...
    async def decide_action_llm(
        ...
        logger.debug(f"Agent {self.id}: Starting LLM call at step {step_count}")
        ...
        resource_map_str = ", ".join(
            [f"{i + 1}: {name}" for i, name in enumerate(RESOURCE_TYPES.keys())]
        )
...
"""
        ...
        action_schema = {
            ...
            "properties": {
                "action": {
                    "type": "string",
                    "enum": ["move", "mine", "talk", "craft", "offer", "accept_offer"],
                },
                "data": {
                    "oneOf": [
                        {
        ...
        print(
            f"Agent {self.id}: About to call get_json with prompt length: {len(prompt)}"
        )
        ...
        logger.debug(f"Agent {self.id}: LLM response received: {response}")

        # Instead of returning, store the result as the agent's intent
        self.llm_intent = (response.get("action", "move"), response.get("data"))

        logger.debug(f"Agent {self.id}: Intent set to: {self.llm_intent}")
        log_to_frontend(
            f"🧠 Agent {self.id}: LLM response received - action: {response.get('action')}, data: {response.get('data')}"
        )
...
Respond as Agent {self.id} in first person, conversationally:"""
        ...
        logger.debug(f"Agent {self.id}: About to generate conversation text")
        ...
        conversation_text = conversation_response.get(
            "message", f"Agent {self.id}: Thinking about my next move..."
        )
        logger.debug(f"Agent {self.id}: Generated conversation: {conversation_text}")
        ...
        if _current_websocket:
            try:
                ...
                logger.debug(f"Failed to send conversation message to frontend: {e}")
        ...
        logger.debug(f"Agent {self.id}: LLM call finished, is_thinking = False")
        log_to_frontend(f"✅ Agent {self.id}: LLM call finished, is_thinking = False")
        self.is_thinking = False
        return log_entry

    def get_fast_action(
        self, trained_policy: object | None, grid: np.ndarray
    ) -> Tuple[str, Any]:
        log_to_frontend(
        ...
        if self.llm_intent:
            ...
            if action in ["talk", "craft", "offer", "accept_offer"]:
                ...
                log_to_frontend(
                    f"🗣️ Agent {self.id}: Returning LLM communication action: {action}"
                )
                return (action, data)
            ...
            # For mine actions, validate before using
                ...
                if (
                    0 <= mine_pos_int[0] < GRID_SIZE_X
                    and 0 <= mine_pos_int[1] < GRID_SIZE_Y
                    and 0 <= mine_pos_int[2] < GRID_SIZE_Z
                    and grid[mine_pos_int[0], mine_pos_int[1], mine_pos_int[2]] != 0
                ):
                    return (action, mine_pos_int)
                else:
                    log_to_frontend(
                        f"🤫 Agent {self.id}: LLM chose to mine invalid block at {mine_pos_int}, moving randomly instead."
                    )
        ...
        if trained_policy:
            ...
            if action_name in ACTION_MAP_MOVE:
                ...
                if (
                    ...
                    log_to_frontend(
                        f"🤫 Agent {self.id}: Policy chose to mine invalid block at {mine_pos_int}, moving randomly instead."
                    )
                    move = random.choice(list(ACTION_MAP_MOVE.values()))
                    return ("move", self.pos + move)
                ...
                log_to_frontend(
                    f"🤫 Agent {self.id}: Policy wanted to talk but no LLM intent - moving randomly instead"
                )
                move = random.choice(list(ACTION_MAP_MOVE.values()))
                return ("move", self.pos + move)
            else:  # fallback case
                log_to_frontend(
                    f"🎲 Agent {self.id}: Policy chose unknown action, moving randomly"
                )
                move = random.choice(list(ACTION_MAP_MOVE.values()))
                return ("move", self.pos + move)
...
class MineCraftEnv:
    def __init__(self):
        ...
        for i in range(NUM_AGENTS):
            ...
            while not spawned:
                ...
                if tries > 1000:
                    logger.warning(
                        f"Could not find a random spawn point for agent {i}. Using fallback."
                    )
                    ...
                    if y_surface != -1 and y_surface + 1 < GRID_SIZE_Y:
                        ...
                        agent.add_to_memory_stream(
                            f"Spawned at position {agent.pos.tolist()} with goal: {agent.goal}",
                            0,
                        )
                        ...
                        agent = Agent(
                            i, np.array([center_x, GRID_SIZE_Y // 2, center_z])
                        )
                        agent.add_to_memory_stream(
                            f"Spawned at position {agent.pos.tolist()} with goal: {agent.goal}",
                            0,
        ...
        for x in range(GRID_SIZE_X):
            for z in range(GRID_SIZE_Z):
                ...
                for i, o in enumerate(octaves):
                    ...
                    if i % 2 == 0:
                        h += (
                            o["amp"] * np.sin(o["freq_x"] * x) * np.cos(o["freq_z"] * z)
                        )
                    else:
                        h += (
                            o["amp"] * np.cos(o["freq_x"] * x) * np.sin(o["freq_z"] * z)
                        )
            ...
            for _ in range(num_veins):
                ...
                for _ in range(vein_size):
                    ...
                    if 0 <= x < GRID_SIZE_X and 0 <= z < GRID_SIZE_Z:
                        current_surface_height = int(
                            np.clip(height_map[x, z], 1, GRID_SIZE_Y - 2)
                        )
    ...
    def _apply_gravity(self, agent: Agent) -> None:
        ...
        x, z = int(agent.pos[0]), int(agent.pos[2])
    ...
    def _get_reward(self, agent: Agent, action: str, data: Any) -> float:
        ...
        if action == "move":
            ...
            if (
                0 <= res_pos[0] < GRID_SIZE_X
                and 0 <= res_pos[1] < GRID_SIZE_Y
                and 0 <= res_pos[2] < GRID_SIZE_Z
            ):
                ...
                if res_idx > 0:
                    ...
                    reward += (
                        res_value if res_value > 0 else 0.1
                    )  # Reward for mining something
                else:
    ...
    def _execute_actions(self, agent_actions: List[Tuple[str, Any]]):
        ...
        for agent, (action, data) in randomized_order:
            if action == "move" and data is not None:
                ...
                agent.add_to_memory_stream(
                    f"Moved from {old_pos.tolist()} to {agent.pos.tolist()}",
                    self.step_count,
                )
                ...
                if (
                    ...
                    if res_idx > 0:
                        ...
                        self.grid[res_pos[0], res_pos[1], res_pos[2]] = (
                            0  # Resource depleted
                        )
                        log_to_frontend(
                            f"⛏️ Agent {agent.id} mined {res_name} at {res_pos}"
                        )  # Debug logging

                        # Record successful mining event
                        agent.add_to_memory_stream(
                            f"Successfully mined {res_name} at {res_pos}. Now have {agent.inventory[res_name]} {res_name}.",
                            self.step_count,
                        )
                        ...
                        agent.update_memory(
                            f"I mined {res_name}. My new goal is {agent.goal}."
                        )

                        # Record goal change
                        if old_goal != agent.goal:
                            agent.add_to_memory_stream(
                                f"Changed goal from {old_goal} to {agent.goal}",
                                self.step_count,
                            )
                        ...
                        log_to_frontend(
                            f"❌ Agent {agent.id} tried to mine empty space at {res_pos}"
                        )  # Debug logging
                        agent.add_to_memory_stream(
                            f"Tried to mine empty space at {res_pos}", self.step_count
                        )
                else:
                    log_to_frontend(
                        f"❌ Agent {agent.id} tried to mine out of bounds at {res_pos}"
                    )  # Debug logging
                    agent.add_to_memory_stream(
                        f"Tried to mine out of bounds at {res_pos}", self.step_count
                    )
                ...
                if data.get("recipient_id") is not None:
                    agent.add_to_memory_stream(
                        f"Sent message to Agent {data.get('recipient_id')}: '{data.get('message')}'",
                        self.step_count,
                    )
                else:
                    agent.add_to_memory_stream(
                        f"Broadcast message: '{data.get('message')}'", self.step_count
                    )
                ...
                if "recipient_id" in data and data["recipient_id"] is not None:
                    recipient = next(
                        (a for a in self.agents if a.id == data["recipient_id"]), None
                    )
                    if recipient:
                        recipient.update_memory(
                            f"Agent {agent.id} said: {data['message']}"
                        )
                        recipient.add_to_memory_stream(
                            f"Received private message from Agent {agent.id}: '{data['message']}'",
                            self.step_count,
                        )
                else:  # Broadcast
                    for a in self.agents:
                        if a.id != agent.id:
                            ...
                            a.add_to_memory_stream(
                                f"Heard Agent {agent.id} say: '{data['message']}'",
                                self.step_count,
                            )
                ...
                if recipe_info:
                    ...
                    if can_craft:
                        ...
                        agent.add_to_memory_stream(
                            f"Successfully crafted {data} using {used_resources}. Now have {agent.inventory[data]} {data}.",
                            self.step_count,
                        )
                        ...
                        agent.goal = random.choice(all_goals)
                        agent.update_memory(
                            f"I crafted a {data}. My new goal is {agent.goal}."
                        )
                        ...
                        for resource, amount in recipe_info["recipe"].items():
                            if agent.inventory.get(resource, 0) < amount:
                                missing_resources.append(
                                    f"{amount - agent.inventory.get(resource, 0)} {resource}"
                                )
                        agent.add_to_memory_stream(
                            f"Failed to craft {data} - missing: {', '.join(missing_resources)}",
                            self.step_count,
                        )
                    ...
                    self.trade_offers.append(
                        {
                            ...
                            "receives": {
                                "item": data.get("item_to_receive"),
                                "amount": data.get("amount_to_receive"),
                            },
                            "status": "open",
                        }
                    )
                    agent.update_memory(
                        f"I offered {amount_to_give} {item_to_give} for {data.get('amount_to_receive')} {data.get('item_to_receive')}."
                    )

                    # Record trade offer event
                    agent.add_to_memory_stream(
                        f"Created trade offer #{offer_id}: give {amount_to_give} {item_to_give} for {data.get('amount_to_receive')} {data.get('item_to_receive')}",
                        self.step_count,
                    )
                else:
                    # Record failed offer attempt
                    agent.add_to_memory_stream(
                        f"Failed to create trade offer - not enough {item_to_give} (have {agent.inventory.get(item_to_give, 0)}, need {amount_to_give})",
                        self.step_count,
                    )
                ...
                if 0 <= offer_id < len(self.trade_offers):
                    ...
                    if offer["status"] == "open" and offer["agent_id"] != agent.id:
                        offering_agent = next(
                            (a for a in self.agents if a.id == offer["agent_id"]), None
                        )
                        if offering_agent:
                            ...
                            if agent.inventory.get(receives_item, 0) >= receives_amount:
                                ...
                                offering_agent.inventory[receives_item] = (
                                    offering_agent.inventory.get(receives_item, 0)
                                    + receives_amount
                                )
                                # Offerer gives items
                                ...
                                agent.inventory[gives_item] = (
                                    agent.inventory.get(gives_item, 0) + gives_amount
                                )
                                ...
                                agent.update_memory(
                                    f"I accepted an offer for {offer_text}."
                                )
                                offering_agent.update_memory(
                                    f"My offer for {offer_text} was accepted."
                                )

                                # Record trade completion events
                                agent.add_to_memory_stream(
                                    f"Successfully accepted trade offer #{offer_id}: gave {receives_amount} {receives_item}, received {gives_amount} {gives_item}",
                                    self.step_count,
                                )
                                offering_agent.add_to_memory_stream(
                                    f"My trade offer #{offer_id} was accepted: gave {gives_amount} {gives_item}, received {receives_amount} {receives_item}",
                                    self.step_count,
                                )
                            else:
                                # Record failed trade attempt
                                agent.add_to_memory_stream(
                                    f"Failed to accept trade offer #{offer_id} - not enough {receives_item} (have {agent.inventory.get(receives_item, 0)}, need {receives_amount})",
                                    self.step_count,
                                )
                        else:
                            # Record failed trade attempt - offering agent not found
                            agent.add_to_memory_stream(
                                f"Failed to accept trade offer #{offer_id} - offering agent not found",
                                self.step_count,
                            )
                    else:
                        # Record failed trade attempt - invalid offer
                        agent.add_to_memory_stream(
                            f"Failed to accept trade offer #{offer_id} - offer closed or invalid",
                            self.step_count,
                        )
                else:
                    # Record failed trade attempt - offer doesn't exist
                    agent.add_to_memory_stream(
                        f"Failed to accept trade offer #{offer_id} - offer doesn't exist",
                        self.step_count,
                    )
    ...
    async def step(self):
        ...
        self.trade_offers = [
            offer for offer in self.trade_offers if offer.get("status") == "open"
        ]
        ...
        print(
            f"Step {self.step_count}: Current thinking count: {current_thinking_count}"
        )

        for agent in self.agents:
            if current_thinking_count >= max_concurrent_llm:
                logger.debug(f"Max concurrent LLM calls reached ({max_concurrent_llm})")
                break

            # Reduce LLM call frequency - every 8 steps instead of 5
            steps_since_last_llm = self.step_count - agent.last_llm_step
            print(
                f"Agent {agent.id}: steps_since_last_llm={steps_since_last_llm}, is_thinking={agent.is_thinking}"
            )

            if not agent.is_thinking and steps_since_last_llm >= LLM_CALL_FREQUENCY:
                logger.debug(f"Creating LLM task for Agent {agent.id}")
                ...
                async def safe_llm_call(agent_ref):
                    logger.debug(f"Starting safe_llm_call for Agent {agent_ref.id}")
                    ...
                    logger.debug(f"safe_llm_call completed for Agent {agent_ref.id}")
                    return log_entry
        ...
        if len(llm_tasks) > 0:
            logger.debug(f"Step {self.step_count}: Created {len(llm_tasks)} LLM tasks")
            log_to_frontend(
                f"📋 Step {self.step_count}: Created {len(llm_tasks)} LLM tasks"
            )
        else:
            logger.debug(f"Step {self.step_count}: No LLM tasks created")
        ...
        if llm_tasks:
            print(
                f"Step {self.step_count}: Waiting for {len(llm_tasks)} LLM tasks to complete"
            )
            log_to_frontend(
                f"⏳ Step {self.step_count}: Waiting for {len(llm_tasks)} LLM tasks to complete"
            )
                ...
                llm_tasks, timeout=30.0
            )  # Reasonable timeout - calls typically complete in 1.5-2s

            print(
                f"Step {self.step_count}: {len(done)} LLM tasks completed, {len(pending)} still pending"
            )
            log_to_frontend(
                f"✅ Step {self.step_count}: {len(done)} LLM tasks completed, {len(pending)} still pending"
            )
            ...
            for task in done:
                ...
                try:
                    ...
                    logger.debug(f"LLM task result: {log_entry}")
                    if log_entry:
                        print(
                            f"Collected log entry from Agent {log_entry.get('agent_id')}"
                        )
                        log_to_frontend(
                            f"📝 Collected log entry from Agent {log_entry.get('agent_id')}"
                        )
                        ...
                        log_to_frontend(
                            f"Agent {log_entry.get('agent_id')} LLM call completed"
                        )

                        # Also send the conversation text to messages if it exists
                        if log_entry.get("conversation"):
                            print(
                                f"Adding conversation to messages: {log_entry.get('conversation')}"
                            )
                            message_data = {
            ...
            for task in pending:
                agent = task_to_agent.get(task)
                if agent:
                    logger.debug(f"Cancelling pending LLM task for Agent {agent.id}")
                    log_to_frontend(
                        f"❌ Cancelling pending LLM task for Agent {agent.id}"
                    )
                    agent.is_thinking = False  # Reset the thinking flag
                else:
                    logger.debug("Cancelling unknown pending LLM task")
                    log_to_frontend("❌ Cancelling unknown pending LLM task")
                task.cancel()
...
def get_agent_state_vector(agent: Agent, grid: np.ndarray) -> np.ndarray:
    ...
    view[
        pad_x_before : pad_x_before + grid_slice.shape[0],
        pad_y_before : pad_y_before + grid_slice.shape[1],
        pad_z_before : pad_z_before + grid_slice.shape[2],
    ] = grid_slice
...
def get_valid_actions_mask(agent: Agent, grid: np.ndarray) -> np.ndarray:
    ...
    for i, (action_name, move_dir) in enumerate(ACTION_MAP_MOVE.items()):
        ...
        if (
            0 <= target_pos[0] < GRID_SIZE_X
            and 0 <= target_pos[1] < GRID_SIZE_Y
            and 0 <= target_pos[2] < GRID_SIZE_Z
        ):
