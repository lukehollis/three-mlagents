...
class Pedestrian:
    def __init__(self, ped_id: int, start_pos: np.ndarray, graph: Any):
        self.id = ped_id
        self.graph = graph
...
6. **START NEW BUILDINGS** only if there are very few active projects (less than {
            len(self.pedestrians) // 3
        }) and you have no current project
...
class SimCityEnv:
    def __init__(self):
        ...
        self.running = False
        self.trained_policy: object | None = None

        # Load real city data
        self.location_point = (37.7749, -122.4194)  # Downtown San Francisco
        self.graph = ox.graph_from_point(
            self.location_point, dist=900, network_type="drive"
        )
    ...
    def _create_businesses(self):
        ...
        high_degree_nodes = [
            node for node, degree in self.graph.degree() if degree >= 3
        ]
        business_nodes = random.sample(
            high_degree_nodes, min(NUM_BUSINESSES, len(high_degree_nodes))
        )
    ...
    def _create_traffic_lights(self):
        ...
        selected_intersections = random.sample(
            intersections, min(8, len(intersections))
        )
